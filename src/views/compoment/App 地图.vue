<template>
  <div class="map">
    ...
    <div class="button">
      ...
      <button @click="endEditorPolyline">结束编辑Polyline</button>

      <button @click="createPolyOn">新建PolyOn</button>
      <button @click="endEditorPolygon()">结束编辑Polygon</button>
      <button @click="close">关闭</button>

      <button
        @click="
          () => {
            router.push('/waterFlow');
          }
        "
      >
        跳转
...
// 新增一个 线段编辑器
let polyLineEditor = reactive({});
// 新增一个 多边形编辑器
let polygonEditor = reactive({});

// 初始化 地图
const initMap = () => {
  // 搜索 会用到这个地图key 安全秘钥
  window._AMapSecurityConfig = {
    securityJsCode: "91b21cbb160f6a931774a5b5c0d691e8",
  };
  AMapLoader.load({
    key: "ea32c02898d933ff97c72873061dc0c0", // 申请好的Web端开发者Key，首次调用 load 时必填
...
const doSearch = () => {
  ...
  placeSearch.search(searchInfo.name, (status, result) => {
...
const createPolyline = () => {
  polyLineEditor.setTarget();
  polyLineEditor.open();
};

// 结束编辑
const endEditorPolyline = () => {
  // 此处设置最终颜色
  polyLineEditor.on("end", (e) => {
    e.target.setOptions({
      strokeColor: "#87CEEB", // 更改颜色为蓝色
      strokeWeight: 5, // 更改宽度为 5px
      strokeOpacity: 0.8, // 更改透明度为 0.8
